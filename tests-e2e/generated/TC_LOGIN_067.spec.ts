/**
 * Assembled from a Playwright Codegen recording of TC_LOGIN_067.
 *
...
test.describe('Login', () => {
  test('TC_LOGIN_067 - Line Chart : Getting flat — #tc_summary_639978 contains "Line Chart : Getting flat line for Weekly and monthly periodicities in the rendered line chart and also displaying wrong data points using Summation operator in Device Data source."', async ({ page, loginPage, projectsPage, bugasuraCredentials, step }) => {
    requireCredentials(bugasuraCredentials);

    await trace({
      testCaseId: 'TC_LOGIN_067',
      module: 'Login',
      scenario: 'Line Chart : Getting flat — #tc_summary_639978 contains "Line Chart : Getting flat line for Weekly and monthly periodicities in the rendered line chart and also displaying wrong data points using Summation operator in Device Data source."',
    ...
    await step('click Line Chart : Getting flat', async () => {
      await (page.locator("#bugReport-table .bug-report__summary--text.hidden-xs").getByText("Line Chart : Getting flat line for Weekly and monthly periodicities in the rende")).click();
    });
  });
});
