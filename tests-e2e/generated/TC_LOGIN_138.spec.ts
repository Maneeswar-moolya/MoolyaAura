/**
 * Assembled from a Playwright Codegen recording of TC_LOGIN_138.
 *
 ...
 * semantic journey instead (action method + recorded assertion phrase + screen). The cell itself is unchanged.
 */

import { expect, requireCredentials, test, trace } from '../fixtures';

test.describe('Login Test Cases', () => {
  test('TC_LOGIN_138 - Verify heading contains "Team work" after panel in the Notifications panel', async ({ page, loginPage, projectsPage, workspacePage, bugasuraCredentials, step, notificationsPanel }) => {
    requireCredentials(bugasuraCredentials);

    await trace({
      testCaseId: 'TC_LOGIN_138',
      module: '',
      scenario: 'Verify heading contains "Team work" after panel in the Notifications panel',
    ...
    await step('#ap_notifications_panel contains "Master Preference"', async () => {
      await expect(await notificationsPanel.panel()).toContainText('Master Preference');
    });
    ...
    await step('heading contains "Team work"', async () => {
      await expect(page.getByRole('heading')).toContainText('Team work');
    });
  });
});
