/**
 * Assembled from a Playwright Codegen recording of TC_SMOKE_037.
 *
...
test.describe('Test Cases', () => {
  test('TC_SMOKE_037 - My Support Cases â€” heading contains "My Support Cases"', async ({ page, appCredentials, step, initialLandingPage, mySupportCasesPage, ssoauthLoginPage, usEnHomePage }) => {
    requireCredentials(appCredentials);

    await trace({
      testCaseId: 'TC_SMOKE_037',
      module: '',
      scenario: 'My Support Cases â€” heading contains "My Support Cases"',
    ...
    // @aura-step {"recordingStepKey":"assertion:0","label":"heading contains \"My Support Cases\"","pageObject":"MySupportCasesPage","method":"headingState","locator":"page.getByTestId('msc-case-list-panel').getByRole('heading')"}
    await step('heading contains "My Support Cases"', async () => {
      await expect(await mySupportCasesPage.headingState()).toContainText('My Support Cases');
    });
  });
});
