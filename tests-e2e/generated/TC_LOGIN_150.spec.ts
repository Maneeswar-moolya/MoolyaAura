/**
 * Assembled from a Playwright Codegen recording of TC_LOGIN_150.
 *
...
test.describe('Login Test Cases', () => {
  test('TC_LOGIN_150 - Verify create project button on the Projects page', async ({ page, loginPage, projectsPage, bugasuraCredentials, step }) => {
    requireCredentials(bugasuraCredentials);

    await trace({
      testCaseId: 'TC_LOGIN_150',
      module: '',
      scenario: 'Verify create project button on the Projects page',
    ...
    await step('Enter the Enter a Project Name', async () => {
      await (await projectsPage.projectNameField()).fill('Test3');
    });
    ...
    await step('Click Faclon', async () => {
      await (await projectsPage.projectButton('Faclon')).click();
    });

    await step('click Desktop Web', async () => {
      await (page.getByRole('img', { name: 'Desktop Web' })).click();
    });
