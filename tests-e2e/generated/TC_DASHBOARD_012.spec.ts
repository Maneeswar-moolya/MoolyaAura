/**
 * Assembled from a Playwright Codegen recording of TC_DASHBOARD_012.
 *
...
test.describe('DashBoard', () => {
  test('TC_DASHBOARD_012 - #tr_637446 > .tabulator-cell.tabulator-cell--checkbox > .rounded-checkbox-cont > .rounded-checkbox-ui — 639978 is not ticked', async ({ page, loginPage, projectsPage, bugasuraCredentials, step }) => {
    requireCredentials(bugasuraCredentials);

    await trace({
      testCaseId: 'TC_DASHBOARD_012',
      module: '',
      scenario: '#tr_637446 > .tabulator-cell.tabulator-cell--checkbox > .rounded-checkbox-cont > .rounded-checkbox-ui — 639978 is not ticked',
    ...
    await step('click #tr_637446 > .tabulator-cell.tabulator-cell--checkbox > .rounded-checkbox-cont > .rounded-checkbox-ui', async () => {
      await (page.locator('#tr_637446 > .tabulator-cell.tabulator-cell--checkbox > .rounded-checkbox-cont > .rounded-checkbox-ui')).click();
    });

    await step('637446 is ticked', async () => {
      await expect(page.locator(".tabulator-row").filter({ hasText: "LineChart: Data labels are not displaying after toggle on" }).locator(".bugChecked")).toBeChecked();
    });
  });
});
