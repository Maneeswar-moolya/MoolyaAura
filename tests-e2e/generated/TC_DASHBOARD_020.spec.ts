/**
 * Assembled from a Playwright Codegen recording of TC_DASHBOARD_020.
 *
...
test.describe('DashBoard', () => {
  test('TC_DASHBOARD_020 - .rounded-checkbox-ui — 639978 is not ticked', async ({ page, loginPage, projectsPage, bugasuraCredentials, step, issuesPage }) => {
    requireCredentials(bugasuraCredentials);

    await trace({
      testCaseId: 'TC_DASHBOARD_020',
      module: '',
      scenario: '.rounded-checkbox-ui — 639978 is not ticked',
    ...
    await step('press Email', async () => {
      await (await loginPage.emailField()).press('ControlOrMeta+V');
    });
