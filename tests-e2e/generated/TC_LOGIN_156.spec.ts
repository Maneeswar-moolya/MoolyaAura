/**
 * Assembled from a Playwright Codegen recording of TC_LOGIN_156.
 *
...
test.describe('Login Test Cases', () => {
  test('TC_LOGIN_156 - Verify issue checkbox after reports nav link on the Workspace page', async ({ page, loginPage, projectsPage, workspacePage, bugasuraCredentials, step, issuesPage }) => {
    requireCredentials(bugasuraCredentials);

    await trace({
      testCaseId: 'TC_LOGIN_156',
      module: '',
    ...
    await step('Click .panel.panel-body.new-counts > .u-dis-f-rc', async () => {
      await (await workspacePage.newIssuesBadge('8 NEW')).click();
    });

    await step('#tc_update_status_1755479 contains "New"', async () => {
      await expect(page.locator("#bugReport-table .badge").filter({ hasText: "New" }).nth(2)).toContainText('New');
    });
