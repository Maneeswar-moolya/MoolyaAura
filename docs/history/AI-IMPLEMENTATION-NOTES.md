...
Archived during application cleanup. Measurements, old project names, corpus counts and
file paths below describe earlier states; they are engineering provenance, not installed
application configuration or current instructions. Current authority is AGENTS.md and governance.
...
### Candidates are generated, budgeted and RANKED (P5)

`candidateSelectorsFor` used to emit CSS shapes only — id, classes, ancestor ids, text —
so `getByRole`, `getByLabel`, `getByPlaceholder` and the rest existed in the SCORING table
and could never be produced as candidates. Measured over the whole corpus of 1058 recorded
targets before anything was changed: Flipkart's `Mobiles` link built **zero** candidates of
any kind, `identityMatched` was absent on **all 1058** recorded locators, and 137 targets
with no proven candidate at all carried a recorded locator already counted at exactly one
element in the press's own document.

- **The recorded Codegen locator is a candidate like any other.** `matchCount` has always
  counted it; a count is not an identity. It is measured in `takePreAction` because that is
  the first moment the expression exists — Codegen writes its line after the action —
  guarded by `sameDocument`, checked by `===` against the parked node, and stamped
  `measuredAt: 'press'` because everything else about it is the press. Identity is never
  obtained by asking Codegen again.
- **Two measurement paths, and the split is not a preference.** `__auraMeasure` evaluates a
  CSS selector with `querySelectorAll`, which cannot express `getByRole` at all; putting an
  accessible-name computation in that string would be a second locator engine. So a
  candidate carries `measuredBy: 'page'` or `'expression'`, and the expression half is
  rebuilt with `buildLocator` on the Node side against the same parked node. **The in-page
  measurement and its own cap are therefore untouched** — the new families never reach it.
- **A semantic candidate is press-time or pick-time ONLY.** `measureCandidates` refuses one
  outright: that path runs after the action with no parked node, so it can produce a count
  and nothing else, and a claim-time candidate is promoted on its count alone. Letting
  `getByRole(...)` in there would promote a semantic locator on cardinality.
- **Generated only where the capture holds the information, with no fallback.** `href` is
  not captured, so no href candidate; `alt` is read into the capture's `attributes` and
  dropped, so no `getByAltText`; `<label>` text is not captured, so `getByLabel` comes only
  from an `aria-label` and **an input has no derivable accessible name at all** — which is
  why `#email_field` gets `getByRole('textbox')` unnamed (30) and not
  `getByRole('textbox', { name: 'Email' })`, however obviously right that looks beside
  `placeholder="Enter your email"`.
- **No page-wide `getByText`.** The content family already refuses it; the semantic family
  refuses it for a sharper reason now that ranking exists — a bare text locator scores 75,
  above an authored id at 70, so generating one lets ranking prefer it on the strength of
  text that happened to be unique when it was recorded. That is TC_LOGIN_128 exactly (one
  `h2` at record time, three at run time). Only the scoped form is emitted.
- **A family budget, not a bigger cap.** Adding these families takes 701 of 1026 captured
  graphs past `MAX_CANDIDATES`, and one larger number resolves that by letting whichever
  family generates most win — which is `structural`, the weakest, because a page with many
  classes produces many class shapes. So `MAX_CANDIDATES = 16` still bounds the structural
  half **and that half is byte-identical to what it was**, while `FAMILY_BUDGET` allows 5
  semantic, 4 semantic-scoped and 3 attribute on top, under a `MAX_TOTAL_CANDIDATES = 28`
  ceiling. Nothing proven today can be displaced by anything added.
- **`provenCandidate` ranks; it no longer takes the first.** The filter is `provesIdentity`,
  unchanged — nothing reaches the ordering that would not have reached `.find()`. Ranked on
  `scoreExpression`, which is `assessLocator`'s own scorer extracted into `scoreChain`, so a
  locator is worth here exactly what it is worth everywhere else.
- **Ties break on the SIMPLER chain, and the corpus caught the first attempt getting this
  wrong.** Breaking on the strongest segment put
  `page.locator("#create_team_invite_form").getByText("Cancel")` (weakest 70, best 85) above
  `page.locator("#create_team_cancel_btn")` (70, one segment) — a chain earning its place
  from a segment that is not the one holding it up — and TC_LOGIN_126/127 lost the Page
  Object reuse they had. Scoping is already paid for inside the score, as the +10 a
  narrowing segment receives.
- **`findMethodByProvenLocator` walks the whole ranked list.** It read the top candidate
  alone, which was harmless while that was generation order and generation put the authored
  id first. Ranking changed which one comes back, and a resolver that sees one expression
  cannot notice that another proven candidate names the very method it is looking for.
  Nothing is relaxed: each candidate still passes `provesIdentity`, still may not be
  positional or built on a generated id, and two claimants are still refused.
- **`GENERATED_CLASS` no longer calls an authored class a bundler class.** The
  `[a-z]+-[a-z0-9]{6,}` arm treated any hyphenated name with a six-character tail as
  generated, so `.login-submit`, `.js-password-input`, `.error-container` and `.mdl-button`
  all took -25. A generated suffix is a HASH — it carries a digit — so the arm now requires
  one. Inert while nothing ranked; load-bearing the moment `provenCandidate` does.
- `npx tsx ai/autocode/semantic-candidates.fixture.ts` — offline, no browser, no model, and
  it writes nothing anywhere.

### A new application bootstraps its first Page Object (P12B)

MoolyaAura could always REUSE an application's knowledge and never CREATE the first piece
of it. Every rule in `resolveOwner` reads a knowledge file, `OWNER_UNKNOWN` is not a
question a resolver may be asked, and `applyProposals` refuses a proposal whose owner no
knowledge file declares - so an application with no knowledge produced raw locators
forever. Flipkart was the live proof: 0 Page Objects, 0 knowledge files, 1 generated spec,
5 raw locators, 0 reuse.

The gap was two links, and both are closed with what the recorder already captures:

- **A FIFTH owner rule, last** (`bootstrapOwner` in `abstraction/propose.ts`). It derives
  an owner from two facts and no others: the ACTIVE APPLICATION and the ROUTE THE DOCUMENT
  STATED at the press. `canonicalIdentity` turns them into `newapp__checkout`, and the
  owner is that slug in PascalCase plus `Page` - `HomePage` for the root route, which is
  what an application's entry screen is called everywhere. Nothing is derived from a page
  title, the DOM, the text, an accessible name, a locator or a model.
- **The first knowledge FILE** (`renderKnowledgeFile` + the hook in `applyProposals`). The
  writer could already create the class, seed the fixtures module and register the fixture;
  what it could not do was bring a screen's knowledge into existence. It is created inside
  the existing transaction, so a batch that fails takes it with it.

- **BOOTSTRAP IS THE EMPTY-KNOWLEDGE FALLBACK AND NOTHING ELSE.** Containment, declared
  container, declared component and declared route all answer first; an AMBIGUOUS declared
  owner stays ambiguous. And the declared rules are now asked with the press-time route as
  well as the reconstructed one (`declaredForRoute`), because a screen that already has
  knowledge must be answered by that knowledge whichever route reached it - without that,
  a recording whose `goto` the step walker cannot read had its SECOND element bootstrapped
  on a screen it had already described.
- **`route` is a new optional evidence field**: `location.pathname`, read in the page, in
  the document that parked the press. Pathname only - a query and a fragment are where a
  token lives - and a path segment that looks like a secret drops the whole route
  (`routeCarriesSecret`). It is never an application identity: that is the locked scope.
- **FAIL CLOSED, AND THE REFUSALS ARE THE POINT.** No route, a route carrying a generated
  identifier (`/issues/636432` names a record, not a screen), a recording whose origin
  names another application, or a recording with no origin at all - each is a refusal with
  no file, no class and no fixture written. A historical recording is never enriched to
  make it bootstrapable; re-recording is the route.
- **Nothing else moved.** Bugasura: 91 proposals, 458 reuses, 207 unmeasured - byte
  identical. Flipkart: identical statuses and owners; only the refusal SENTENCE is more
  precise (`BOOTSTRAP_ROUTE_UNKNOWN` where it was `OWNER_UNKNOWN`, both SEMANTIC, both
  unaskable, both `PROPOSAL_REJECTED`).
- `npx tsx ai/projects/bootstrap-knowledge.fixture.ts` - 37 offline checks in a temporary
  artefact root, no browser and no model. Ten mutations, all of which bite.
- **A bootstrapped screen GROWS the same way, and that behaviour is now pinned too.** A
  second proven capability on an established screen is APPENDED - the existing entry stays
  byte-identical, the class keeps its methods, the fixture is registered once - and a
  recording of an element that is already wrapped is a REUSE however it is renamed or
  re-measured. New evidence about an established capability is never applied: a rival
  proven locator for a known element is written nowhere. `npx tsx
  ai/projects/knowledge-enrichment.fixture.ts` - 22 offline checks, five mutations, all of
  which bite. Several of these invariants are guarded twice over (a reuse resolver answers
  before a proposal is made, and the writer refuses again on disk), so each mutation is
  aimed at whichever gate actually decides in that scenario.
- **The evidence that is refused is now WRITTEN DOWN.** A recording that proves a second
  expression for an element the repository already wraps used to leave no trace at all:
  the capability stayed correct and the observation was lost. It is now recorded - the
  established capability, what knowledge declares, what this recording proved, verbatim -
  in `ai/reports/abstraction-alternatives.jsonl`, and **nothing reads it**. No writer, no
  proposal, no locator. The decision it supports belongs to a person.
  - **ONLY WHERE THE ELEMENT IS PROVEN THE SAME ONE.** The analyser'''s duplicate gate knows
    two things at once - this element is already wrapped, and this recording proved
    something else for it - so it is the only source. A reuse the matcher resolved by
    accessible NAME is not element identity, and recording one described a modal'''s close
    control as new evidence for an unrelated capability until the fixture caught it.
  - **NO SCORE, NO VERDICT, NO TIMESTAMP.** Quality is not capability ownership, so no
    comparison is stored - a stored comparison is read as a decision by whoever automates
    next. The timestamp went the same way for a duller reason: it made the report'''s bytes
    differ on every run over identical evidence, which is the idempotency rule broken by
    the one field with no consumer.
  - An instantiated parameter template is the capability being USED, not a rival for it,
    and a declared strategy written as prose states no locator to differ from. Both are
    refused. Measured over the Bugasura corpus: 2 observations from 101 recordings.
- **The writer fails closed, and proves what it left behind.** Both halves wrap the
  EXISTING mutation boundary - there is no second writer and no second transaction.
  BEFORE a capability is written,  asks the same questions the
  analyser asked, independently, because the writer is the last thing between a proposal
  and the repository: a capability must be named, every file the write would touch must
  lie inside the ACTIVE application (, which had no production caller until
  now), the locator must be a measured expression rather than prose,  must
  hold for the press-time proof, and the write must neither change what an established
  capability resolves to nor give one element a second name. Every refusal joins the
   array the batch already refuses on, so one bad proposal still writes nothing.
  AFTER a permitted write, the existing  step also asks whether every capability
  that existed still declares what it did, and whether any file this run did not intend to
  write has changed - compared as BYTES against a snapshot taken before the write, never
  by mtime or ordering. A failure takes the existing rollback, which restores every edited
  file and removes every created one.
  - **A post-write failure cannot currently be provoked end to end**, because every defect
    that is reachable is refused before the write. That is the fail-closed property
    working rather than a hole, and it is why the verification is exported and proven
    against a deliberately corrupted result instead.

### "This capability already wraps this element" is a MEASUREMENT (P13.6)

`existingCapability` in `abstraction/propose.ts` answers one question - is the element that
was just recorded already wrapped by a capability the repository has? - and for a year it
answered it by tokenising the proposed locator, taking the LAST `#id`/`.class` token, and
returning the first knowledge entry declaring that token. Phase 13.5 measured that basis
and it is not identity in either direction:

- **9 associations on the Bugasura corpus, of which 3 were provably wrong.**
  `#first_report_modal .mdl-button` filtered to `close`, and `#actual_bug_add_sec
  .mdl-button` filtered to `Cancel`, were each reported as `WorkspacePage.newIssue()` -
  declared `#bug_no_issue_added .mdl-button` filtered to `add Add Issue`. Different
  container, disjoint text filter, same trailing class. A person had already hand-written
  `#first_report_modal button:has-text("close")` on a different class for one of them.
- **1 more was wrong on the recorded evidence, 4 proved nothing, 1 was true and already
  caught by the name gate.** Nothing the gate did on that corpus was load-bearing:
  disabling it entirely left `PROPOSED` empty, so it prevented no write. What it did do was
  move 8 elements out of the review queue - `lifecycle.ts` turns `REUSE` into
  `EXISTING_PO_REUSED` / `passed` / no remedy - and manufacture 2 false evidence records.
- **The premise itself is false for a whole shape of expression.** `getByText(...)`,
  `getByRole(...)` and a bare tag contribute no token, so for
  `page.locator("#all_apps").getByText("Faclon labs")` the "last concrete token" is the
  CONTAINER. The gate was comparing a scope against an element.
- **And it is blind in the other direction**: one element recorded once by id and once by
  role shares no token, so a SECOND capability was written for it. Reproduced in a synthetic
  application with no Bugasura DOM - both halves generalise.

**The basis is now a measurement taken at the interaction.** At every press and every pick
the recorder resolves the ESTABLISHED CAPABILITIES' own declared locators against the parked
node (`measureDeclaredCapabilities`, built on the existing `measureExpressionCandidates` -
same rebuild, same document guard, same in-page `===`, same `provesIdentity` bar) and files
the answers in `TargetEvidence.capabilityMeasurements`, each carrying the capability it was
taken for. `existingCapability` reads those. There is no second identity system and no
fallback: **if identity was not measured, no capability is claimed.**

- **A FOURTH evidence list, deliberately not one of the other three.** `derivedCandidates`,
  `positionProvenCandidates` and `rejectedCandidates` hold locators the framework might
  EMIT, and every one of them competes for a family budget and a rank. These are never
  emitted, ranked, budgeted or promoted, so mixing them in would change what is generated.
  Nothing about candidate generation, budgets, scoring or ranking moved.
- **An ordinary candidate measurement counts too, when it IS the declared locator.** If the
  generator happened to build the capability's own expression and the browser proved it at
  the press, that is a measurement of that capability's locator taken for another reason.
  This is what lets HISTORICAL recordings still resolve: measured over the corpus, 7 such
  proofs exist across 4 capabilities, including `ProjectsPage.createTeamCancelButton` -
  the entry that was UNREACHABLE to every resolver before its `locator_strategy` was added.
- **A PARAMETERISED capability is matched on its template**, which is
  `findParameterisedMethod`'s existing structural rule rather than a second one: the
  instantiated expression was proven at the interaction, and a declared template equal to
  the proposed one instantiates to exactly that expression. An instantiated row is the
  capability being USED, never a rival for it.
- **Two claimants are no claimant.** The old gate returned the FIRST entry declaring the
  token and said nothing about the second; `#ap_notifications_panel` is declared by both
  `NotificationsPanel.panel` and `NotificationsPanel.settingsButton`, and which one was
  reported was file order. Ambiguity is refused here as it is everywhere else.
- **Corpus effect, Bugasura, same 409 recordings**: element-gate claims 9 -> 1, retained
  alternatives 2 -> 0, `REUSE` 14 -> 6, `NEEDS_REVIEW` 66 -> 74. Proposals (91), matcher
  reuses (458) and unmeasured targets (207) are unchanged, and `PROPOSED` is empty before
  and after - so nothing new is written and nothing that was written has moved. The one
  surviving claim is the row-scoped `.bugChecked` group against
  `IssuesPage.issueCheckboxState`, on the template basis. Flipkart is unchanged, and by
  inspection must be: its one declared capability is a `getByRole` chain, which yields no
  selector token at all, so the old gate could never have matched it either.
- **The honest cost.** A recording that carries no capability measurement AND no proven
  candidate equal to the declared locator now leaves identity UNPROVEN, so a genuine
  duplicate can reach the proposal path where the token gate would have blocked it. That is
  the governed outcome - unknown is not yes, and the review path decides - and it shrinks
  with every recording made from now on, because every press measures the capabilities.
- `npx tsx ai/projects/knowledge-enrichment.fixture.ts` - 67 offline checks in a temporary
  artefact root with two registered applications, section J being I1-I14. Six mutations, all
  of which bite. A seventh - attributing alternative evidence without proven identity - was
  written, applied and measured, and CANNOT bite: attribution has no gate of its own, it
  reads the identity gate's answer, and the only name-resolved items are resolved by
  `findMethod` before the analyser ever sees them. M1, M2 and M6 each corrupt identity and
  each turn the attribution checks red, which is the same invariant reached from the
  reachable side.

### A validated terminal answer ends the exchange (P13.7)

`MAX_RESOLVER_ATTEMPTS` is 3, and a case that engaged the resolver cost ~4-9 minutes:
measured over the recorded exchanges, the transport is **99.97% of such a case**, at ~50 s
per call in August and ~83-102 s in September, with a 120 s timeout ceiling. Every recent
exchange spent all three.

Two of those three calls were often asking a question that had already been answered.
`revalidate` computes `decisionIsTheAnswer` - whether the question put to the resolver is one
whose ANSWER IS THE DECISION (`UNCLASSIFIED_TARGET`, `CONTAINER_OR_CAPABILITY`,
`CONSTANT_PARAMETER_VALUE`) - and its own rejection text says such a decline **"is a correct
answer"**. The loop could not see that: it stopped only on `accepted`, `REUSE` or `REFUSED`,
so a settled decline read as repairable and the identical prompt went out twice more.

- **TERMINAL means the validator proved repair cannot help**, and it is deliberately narrow:
  the resolver declined AND every question still open is one whose answer is the decision.
  A decline that leaves `NO_METHOD_NAME` open is NOT terminal - the repair prompt quotes what
  is missing and a resolver has produced a name on the second attempt before.
- **NOTHING IS ACCEPTED THAT WAS NOT ACCEPTED BEFORE.** A terminal exchange ends exactly where
  a spent budget ends: `NEEDS_REVIEW`, a STRUCTURAL `RESOLVER_EXHAUSTED` refusal, not re-asked
  in this run. Only the transport calls change.
- **A TIMEOUT IS NOT AN ANSWER.** The transport-failure path is untouched: it is an outage,
  it spends nothing, it leaves the proposal exactly as the deterministic engine produced it,
  and it is never terminal. Making it one is mutation R3, and R3 bites.
- **A settled answer is settled next run too.** `exhaustedBefore` reads the recorded
  `terminal` flag, so the saving lands on a corpus that is analysed repeatedly rather than
  only within one run. An outage still memoises nothing.
- **Measured by replaying the REAL recorded first attempts through the real validator**: over
  the last 12 exchanges, **32 transport calls become 24**. TC_SMOKE_013 goes 3 calls -> 1
  (~278 s -> ~93 s); TC_SMOKE_016 goes 9 -> 7. What remains in TC_SMOKE_016 is two
  `TEXT_ONLY_IDENTITY` exchanges that legitimately repair - the resolver named a Page Object
  class that does not exist, which a repair can fix - and then hit the 120 s transport
  ceiling. That is a transport cost, not a resolver-logic one.
- **The resolver was already correctly scoped** (`ensurePageObjects` passes only the proposals
  belonging to the case, by `testCaseId` or sighting). There is no corpus-wide semantic path.
  `analyseCorpus` IS corpus-wide and costs 2.2-2.8 s on Bugasura, 0.08-0.13 s on Flipkart.
- **The resolver now says what it spent.** Every attempt records `transportMs` and whether
  anything `responded`; every exchange records `startedAt`/`finishedAt`/`totalMs` and whether
  it was terminal; the attempt metrics carry a `semantic` block (exchanges, calls, attempts,
  totalMs, accepted, rejected, terminalStops). The observability gap was total: a case spent
  278 s here while `agentMs: 0`, `generationMs: 0`, gate `null` and
  `model: "none (deterministic)"`. **The metrics SCHEMA is deliberately NOT bumped** - it is
  folded into `frameworkFingerprint`, so a bump grants every row a fresh attempt budget, and
  this block decides nothing.
- `npx tsx ai/autocode/abstraction/semantic.fixture.ts` - section T, 10 checks. Five
  mutations (R1-R5), all of which bite.

### A bootstrapped Page Object imports the framework from where the framework IS (P13.7)

TC_SMOKE_014 produced correct deterministic Page Object work - the class, the method, the
knowledge entry, the fixture - and Playwright refused the whole spec at COLLECTION:

    tests-e2e/pages/flipkart/home.page.ts:14  Cannot find module './base.page'

`renderPageObjectClass` emitted two FIXED relative imports, `./base.page` and
`../support/resilient-locator`. Both are correct for exactly one layout: a class written
directly into `tests-e2e/pages/`, which is the legacy unscoped layout Bugasura holds. Every
other application's classes live one directory deeper, so both paths named files that do not
exist - and the second one was broken too, silently, behind the first.

- **Generalized, not per-application.** The two specifiers are computed from where the class
  will be written to where the framework file actually is, anchored on the fixtures module's
  own directory - the layout root the scope already resolves. `BasePage` and the healing
  recorder are shared framework capabilities, which the architecture contract permits
  resolving across scopes; what was not permitted was guessing where they are.
- **The legacy layout is byte-identical**: the computation produces `./base.page` and
  `../support/resilient-locator` for a class in `tests-e2e/pages/`, which is what was
  hard-coded. Nothing that exists changed.
- **The already-written file was repaired** - two import lines in
  `tests-e2e/pages/flipkart/home.page.ts`, no capability, locator or identity touched - and
  the Flipkart spec now collects: `Total: 1 test in 1 file`, where it previously could not
  resolve its own base class.
- `npx tsx ai/projects/bootstrap-knowledge.fixture.ts` - section F, 6 checks, including a
  THIRD synthetic application to prove the rule is not a second special case. Three mutations
  (B1-B3), all of which bite.

### Every strategy the framework emits has a contract (P12A)

A locator strategy used to be a bare string spelled at four unrelated places - the
emission site, `familyOf`, the scorer's shape classifier and `buildLocator`'s rebuildable
set - and every disagreement between them failed **silently, in the same direction**: the
new strategy quietly vanishing. Measured, all three:

- an unmapped family falls to `structural`, which carries no `FAMILY_BUDGET` entry, so
  `applyFamilyBudget` admits **none** of it;
- an expression `scoreExpression` cannot parse scores `null` and therefore ranks **last**;
- an expression `buildLocator` cannot rebuild is measured `matchCount: null`, so it can
  never be proven and never be selected.

None of those produces an error or a missing-candidate report. `STRATEGY_CONTRACT` in
`dom-evidence.ts` now declares, for every strategy, its **family**, its **budget**
(`structural-cap` / `family-budget` / `unbudgeted`), its **measurement path**
(`page` / `expression`), whether it is **scoreable** and whether it is **rebuildable**.
`familyOf` reads that table and keeps every answer it ever gave, including its default for
a name nothing declares.

- **The table decides nothing new.** Only `family` is read at run time, which is the one
  answer `familyOf` always returned; the other three fields exist so that an omission is a
  red test rather than a candidate nobody notices is missing. Measured over all 1041
  captured graphs: candidate lists, families, measurement paths and all 1247 ranked
  selections are byte-identical before and after.
- **Rebuildability is required only of an `expression` strategy**, and the contract records
  where it is genuinely absent: `container-text`, `scoped-class-text` and
  `scoped-class-pair-text` build `.filter({ hasText })`, which `buildLocator` deliberately
  refuses to reconstruct - and never has to, because those three are counted in the page by
  `__auraMeasure`. That was discovered by the contract, not assumed by it.
- **Adding a strategy is one line plus its emission site.** `strategy-contract.fixture.ts`
  runs the real generators over synthetic graphs, and an emitted strategy with no
  declaration fails by NAME. A declared strategy nothing emits fails too, unless it is in
  the fixture's own `NOT_YET_EMITTED` list - which holds the four reserved names `familyOf`
  has always answered for and `recorded-locator`, Codegen's own line, built in the recorder
  rather than by a generator.
- Cost: `familyOf` is 13.9 ns → 19.5 ns per call (min of 5 x 1,000,000), against ~17 calls
  per captured target. The declaration is read through a `Map` built once at module load.
- `npx tsx ai/autocode/strategy-contract.fixture.ts` - 9 offline checks over 30 emitted
  strategies, no browser and no model. Eight mutations, all of which bite, including a new
  emission site with no contract entry.

### A name matched exactly and the same name matched loosely (P10)

`getByRole(role, { name, exact: true })` and `getByRole(role, { name })` are **two
candidates built from one piece of evidence**, and both are now generated. They fail in
opposite directions and each failure is measured, not argued — across 20 pressed targets
on two applications, 15 were valid both ways, 1 exact-only, and one transition was
loose-only:

- **Exact fails when the application EXTENDS a name.** Bugasura's password field computes
  as `Password` and, once its validation error shows, as `Password Not too short! enter
  min 5 characters.` — the exact locator then matches **0** while the loose one still
  matches 1. That is the Phase 9 finding stated precisely: the hazard was never "the name
  changed", it was `exact: true` against an appended name.
- **Loose fails when another element's name CONTAINS this one.** `{ name: 'Sign In' }`
  matches **2** on the same page — Google's sign-in button contains the words — where
  `exact: true` matches 1. Refused at the press by the ambiguity gate that was already
  there, in the same run. This is the hazard `exact: true` was added for in P5.1, where
  the case was Flipkart's "Home"; re-measured, that page now resolves to one element
  either way, and Bugasura's Sign In is where the ambiguity actually reproduces.

So neither is preferred and nothing new decides anything: each is counted at the press,
each must resolve to exactly one element, and each must BE the element acted on.

- **The loose twin is emitted LAST within its family, and the position is the whole
  safety property.** Beside its exact twin it displaced 150 candidates the generator
  already produced (125 scoped text, 25 scoped label) over the 1026 captured targets —
  none proven today, and `FAMILY_BUDGET` exists precisely so that "not proven yet" is not
  treated as "worthless". Appended, it displaces **zero** and still offers 374 of the 524
  loose candidates. No budget number moved: `MAX_CANDIDATES` is 16 and
  `MAX_TOTAL_CANDIDATES` 28, as before.
- **A loose twin may not outrank its exact twin** (`LOOSE_STRATEGIES` in `classify.ts`).
  Both score 95/95 over one segment, so the shorter-expression tie-break would have
  promoted the loose form on every element where both are proven — a global preference
  arrived at by counting characters, and one no measurement supports. With the twin rule
  in place the ranked order over the whole archived corpus is **byte-identical**: 1231
  ranked lists, 0 differing, so no selection made today changes and Page Object reuse
  stays exactly where it was. The addition can only answer where the exact form is refused.
- **Whether the loose form is DURABLE is a separate question, and it is not settled here.**
  It survived the one transition exact did not, and exact survived one on which loose was
  already ambiguous. Ranking on durability would need durability measured, which the
  recorder does not do — a press is one instant.
- Cost: generation 0.297 → 0.43 ms per target (arithmetic), and at the press, live on
  Bugasura, 15 → 19 ms mean over 5 targets for 19 → 20 candidates measured.
- `npx tsx ai/autocode/exact-nonexact.fixture.ts` — 22 offline checks, no browser and no
  model. Eight mutations, each biting the check it is aimed at.

### Durability: does the candidate still name the same element afterwards (P11)

`accessibleNameStable` asks whether the browser-computed NAME held still.
`classifyDurability` asks the question a locator is actually judged by: **after a real
state transition, does this expression still identify the element that was pressed?**
The two come apart in both directions, which is why neither is derived from the other.

Measured over 19 targets on my.bugasura.io and flipkart.com — production hook,
production press measurement, production `splitCandidates`, the page's own
`__auraMeasure` and `measureExpressionCandidates` for the re-measurement:

- **126 candidates proven at the press → 104 durable, 11 unknown, 10 unresolved,
  1 ambiguous.** The 11 unknown are two reload transitions: the document is gone, so no
  count is about that element any more.
- **Three distinct failure modes, none of them "the name changed".** Exact role+name goes
  to ZERO when the application *extends* a name (Bugasura's email and password fields).
  Non-exact role+name becomes AMBIGUOUS when other elements arrive whose names contain
  it (Flipkart's `Mobiles`: 1 → 3 after a search query, with its name perfectly stable).
  Every TEXT-derived candidate goes to zero when a message is rewritten (Bugasura's
  password error label), while `#password_field-error` stays durable.
- **`name stable + candidate ambiguous` and `name changed + candidate durable` both
  occur**: 13 candidate-measurements are durable through a changed name, and one is
  ambiguous through a stable one. A name verdict predicts nothing about a locator.
- **The deterministic alternatives are not automatically better.** An unscoped
  `[name="email"]` was AMBIGUOUS at the press 7 times — the three simultaneously mounted
  Bugasura forms — while the scoped form was proven 8/8 and durable 10/10 and the
  authored id 5/5. "Prefer an id" and "prefer semantic" are both wrong as global rules;
  what held up was SCOPE.
- **`classifyDurability` is pure and fail-closed**, and one element is never durable on
  its own: not proven at the press, another document, a released or detached slot, or no
  count at all are all `unknown`; 0 is `unresolved`; more than one is `ambiguous` even
  when the target is among the matches; exactly one WITH identity is `durable` and
  exactly one WITHOUT it is `wrong-target`.
- **Collection only, and wired to nothing.** No recording path calls it, no evidence
  field carries its verdict, and no locator is chosen or refused by it. Ranking, budgets,
  AI eligibility, Page Object reuse and the gates are untouched — the phase produced
  knowledge, not a preference.
- Cost, when it is measured at all: the re-measurement is 159 ms median per target for
  every proven candidate (in-page batch 4 ms median for all CSS candidates at once,
  40 ms median per Playwright-expressed candidate on the Node side). No sleep is used
  anywhere: every transition waits on a signal the application itself produces, and a
  transition with no settled point is reported unmeasurable rather than approximated.
- `npx tsx ai/autocode/candidate-durability.fixture.ts` — 25 offline checks, no browser
  and no model. Eight mutations, all of which bite.

### What the capture records, and what it refuses to (P5.1)

The locator engine was made better informed rather than larger. Four fields, **+316 bytes
on a 2,215-byte snapshot (+14%)**, chosen from measurements over eleven live targets — not
from a list of everything a DOM can hold.

- **`href` is the AUTHORED attribute, never the resolved URL.** `node.href` on an anchor
  resolves to an absolute URL carrying the origin, and the same application at two
  environments has two origins — so a locator built on it belongs to a deployment rather
  than to the application. `hrefAbsolute` is a FLAG, not a resolution, and it is what keeps
  `javascript:void(0)` (which names no destination) and cross-origin links out of candidate
  generation with one test. Measured: Flipkart's `Mobiles` and `Home` each resolve to
  exactly one element by their authored path, and it is the only candidate either has.
- **`alt` is captured and its ambiguity is preserved.** 91 images on Flipkart's homepage
  carry one; of eight sampled, three identified their element uniquely and four shared
  `alt="Image"` across 62 elements. A shared alt is not a reason to withhold the candidate,
  it is a reason to measure it — the live check emitted `getByAltText("Image")`, measured 58,
  and refused it. Generation never asks how many elements a candidate will match.
- **The accessible name is the BROWSER'S, through CDP, and the page's guess is marked as a
  guess.** No DOM API exposes a computed accessible name, so the capture records
  `aria-label || title` and stamps `accessibleNameSource` plus
  `accessibleNameVerified: false`; `browserAccessibleName` then replaces it via
  `Runtime.evaluate` → `Accessibility.getPartialAXTree` on `window.__auraTargets[slot]`, at
  the press, before any candidate is derived. **Reached through the registry, not a
  Playwright handle** — that expression is already the one both sides agree names the
  element, and it makes the document guard free: after a navigation the array is fresh and
  the slot is empty. `fetchRelatives: false`, so it is the node alone — 66 bytes.
- **CDP is a CAPABILITY, never a requirement.** One session per page, cached in a WeakMap;
  Firefox and WebKit resolve to null once and are never asked again. A recording is never
  refused for want of it, and a candidate built from an unverified name is measured and
  refused exactly like any other — `provenCandidate` did not move.
- **Only a VERIFIED name outranks the attributes.** Measured: an attribute-based guess
  agreed with the browser on 7 of 10 targets, and all three misses produced locators that
  matched ZERO elements. Bugasura's password field computes as
  `"Password Not too short! enter min 5 characters."` **once its error label is showing** and
  as `"Password"` before — both are correct records of the instant they were taken, and the
  second is what a real recording produces, because a person focuses the field before any
  error exists. An element whose name folds in transient text yields a state-dependent
  role+name locator; the falsification gate catches it (the clean run fails) rather than
  shipping it.
- **Exactly ONE non-qualifying ancestor, the nearest, APPENDED.** The container/role/id rule
  is unchanged and still decides what is kept; the plain wrapper goes after every qualifying
  ancestor, marked `nonQualifying`. Appended rather than inserted because consumers SLICE
  this array — `stableAncestors` takes the first three carrying an id, the container family
  the first four — so an entry placed in depth order can push a qualifying ancestor out of a
  window it was inside. Measured: one plain ancestor gained identity-proven candidates on two
  targets and lost none; **two** lost one on a target, and any rule that reorders or scores
  lost them on two. Flipkart's `Mobiles` went from 1 candidate to 6.
- **The extras are on the TARGET only.** Putting them on every node measured at +232%
  against +14%; an ancestor's href or alt feeds no strategy.
- **An href can be a credential.** A reset link is `?token=…`. `hrefCarriesSecret` refuses the
  whole href — never a trimmed one, because a redacted URL is a different URL that still
  reads like an authored link — on a forbidden parameter NAME or a secret-shaped VALUE, the
  same two rules `filterAttributes` already uses. The path is never tested: `/reset-password`
  is a route.
- `npx tsx ai/autocode/target-snapshot.fixture.ts` — offline, no browser, no model, writes
  nothing. It runs the GENERATED capture string against a stub DOM.

**What was measured and deliberately NOT built.** Full DOM: 474 KB, **214× the snapshot**,
479 MB across the corpus — and the two hard cases were solved by 30 bytes of `href`.
Before/after snapshots: the target's own fields that changed across a real state change were
**none at all**, for +100% size. Sibling capture: no strategy consumes siblings, so more of
them feeds nothing. Page-level accessibility snapshots, `value` capture and broad ancestor
expansion: same answer, and the numbers are in `docs/history/REMAINING.md`.

**Identity did not need any of this.** Across every real look-alike pair on Bugasura — two
password inputs, three email inputs, eleven buttons — the richer snapshot added **no
distinguishing power**, because `id` already separated all of them. Identity is still
`targets[slot] === element` against the parked node, and the four levels stay distinct:
browser node identity (never serialised), snapshot identity (`elementRef`/`captureRef`),
document identity (`__auraDocument`, dies with the document), and logical context
(`origin.applicationId`, the only one safe to compare across runs).
