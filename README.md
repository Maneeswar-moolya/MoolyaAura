# AURA — Excel-driven Playwright automation

> **This repository has two independent things in it.** Everything above the divider is the
> AURA test-automation framework, which is what most people working here need. Everything
> below it is the upstream [Playwright MCP](https://github.com/microsoft/playwright-mcp)
> README, kept intact because this repo is a fork of that package and
> `npm run lint` regenerates part of it.

## 1. What this is

A framework for turning **test cases written in Excel** into **executable Playwright tests in
...
| Piece | Role |
| --- | --- |
| **Playwright** | Runs every test, and provides the recorder the authoring flow is built on |
| **TypeScript** | Every spec, Page Object and tool in `ai/` is TypeScript, run through `tsx` |
| **Excel** | Where test cases are authored: ID, scenario, steps, expected result, priority |
| **AURA pipeline** (`ai/`) | Recording, evidence capture, deterministic code assembly, quality gates |
| **Page Objects** (`tests-e2e/pages/`) | The vocabulary a generated spec is written in |
| **Page Knowledge** (`ai/knowledge/page/`) | What a screen looks like, recorded once and reused |
| **Claude Code** | A *fallback* author for cases deterministic assembly cannot complete |

### The principle worth reading twice

**Deterministic resolution and existing framework knowledge come first; AI is a controlled
fallback for unresolved ambiguity.** A recorded case is assembled with no model involved at all.
A model is asked only when a case cannot be built from what is already known — and whatever it
writes must still survive the same quality gate as everything else.

## 2. Key capabilities

Each of these exists in the repository today:

- **Excel-based authoring** — one row per case, filtered by sheet/module/priority/tag/status
- **Data-driven rows** — cases that execute with no spec written, driven by an `Assert Outcome`
  column
- **Live browser recording** through Playwright's own recorder
- **Assertion picker** — state what must be true *while* recording, from a list of assertions the
  selected element can actually support
- **DOM evidence capture** — a bounded relationship graph per recorded target, measured at the
  moment of the press
- **Deterministic locator resolution** with identity verification: a candidate is used only when
  the browser measured it at exactly one element *and* that element is the one the person acted on
- **Contextual locators** for repeated elements — a container identified by its own text, then the
  element inside it
- **Dynamic/generated ID handling** — numeric and hashed IDs are classified dynamic and never
  become locators
- **Page Object reuse**, with anything unresolved reported rather than invented
- **AI fallback** for cases deterministic assembly cannot complete
- **Quality gates** — every generated spec must pass as written and *fail* with its assertions
  broken
- **Execution and reporting** — per-step evidence, screenshots, traces, HTML and Allure reports,
  results written back into the workbook

## 3. Architecture

```mermaid
flowchart TD
    A[Excel test case] --> B[Parser: test-case model]
    R[Live recording] --> B
    B --> C[Cache, duplicate and quality checks]
    C --> D[Context and page-knowledge routing]
    D --> E[Deterministic resolution]
    E --> F[Existing Page Object method]
    E --> G[Measured locator + identity check]
    E --> H[AI fallback, only when unresolved]
    F --> I[Spec assembly]
    G --> I
    H --> I
    I --> J[Quality gate: passes clean, fails mutated]
    J --> K[Playwright execution]
    J -->|refused| Q[Quarantine]
    K --> L[Results, evidence, reports]
    L --> M[Status promotion and page knowledge]
```

The recording path in detail — including the assertion picker — is in
[RECORDING-GUIDE.md](RECORDING-GUIDE.md).

## 4. Quick Start — Record Your First Test

For a test author. You need the dashboard running and an account in `.env`.
...
Open **http://moolyaautomationreport.com** (or `http://127.0.0.1:4321` if port 80 is
taken - the terminal prints which). Then:

1. Pick a worksheet and a test case.
2. Press **Record**. A browser opens.
3. Do what the test should do - sign in, navigate, click.
4. Press the **Assert** pill (bottom right), click the element you want to check, and
   choose the assertion. **A recording with no assertion is refused**, on purpose.
5. Close the browser or press Stop, review, then **Save**.

`RECORDER_TRANSPORT=live` is what makes step 4 possible. Without it the recorder still
captures actions but there is no assertion picker and no DOM evidence - see
[RECORDING-GUIDE.md](RECORDING-GUIDE.md).

## 5. Quick Start — Author Your First Test

For a test author who would rather write the case than perform it.

1. In the dashboard, **Add** a case (or edit one) and fill in Steps and Expected Result.
2. Save. The workbook row is written.
3. Generate:
...
```bash
npm run excel:test -- --grep TC_LOGIN_050
```

The generator reuses existing Page Object methods wherever it can, and refuses to invent
a locator it cannot prove. A case that comes back **Needs Review** is telling you the
evidence was insufficient, not that it failed - see
[ABSTRACTION-GUIDE.md](ABSTRACTION-GUIDE.md).

### From a recording to a reusable Page Object
...
Every step except one is deterministic. A model is asked a single question, and only when
the deterministic engine reached `NEEDS_REVIEW` for a reason that is about **meaning** —
who owns this element, what should the capability be called, is this one reusable thing —
with **zero** safety refusals present. It cannot choose a locator, invent an owner, or
clear a safety rule, and its answer is re-validated against every deterministic gate before
anything is written. Measured on the current corpus: 4 of 57 proposals are even eligible to
be asked about, and reuse itself never involves a model.

Off unless you ask for it: `npm run excel:autocode -- <workbook> --create-page-objects`.
...
npm install
npx playwright install chromium
cp .env.example .env          # fill in the placeholders; .env is git-ignored
```
...
**Requires Node >= 18** (`package.json` engines). Playwright is pinned in `package.json`; install
browsers with `npx playwright install chromium` rather than assuming they are present.

> A bare `npx playwright test` runs the **upstream MCP suite**, not this one. Use
> `npm run excel:test`.
...
| Document | For |
| --- | --- |
| [FRAMEWORK-GUIDE.md](FRAMEWORK-GUIDE.md) | End-to-end walkthrough: setup, authoring, running, maintenance |
| [RECORDING-GUIDE.md](RECORDING-GUIDE.md) | Recording in a browser, the assertion picker, recorded → spec |
| [ABSTRACTION-GUIDE.md](ABSTRACTION-GUIDE.md) | How a locator becomes a reusable Page Object method, and the one question a model may be asked |
| [EXCEL-AUTOMATION.md](EXCEL-AUTOMATION.md) | Command reference, dashboard, data-driven rows |
| [CLAUDE.md](CLAUDE.md) | Project rules, the two-suite separation, invariants |
| [ai/CLAUDE.md](ai/CLAUDE.md) | Why the generation pipeline is built the way it is |
| [CONTRIBUTING.md](CONTRIBUTING.md) | Upstream contribution process |
| [SECURITY.md](SECURITY.md) | Reporting security issues |

## 8. Credentials and secrets

Credentials live in `.env`, which is git-ignored. `.env.example` is committed and contains
**placeholders only** — never real values. Use a dedicated QA account: the suite makes deliberate
failed sign-in attempts, which can trip lockout on a personal account.

Nothing in this repository should ever contain a real password, token or API key. Recorded
password fields are stored as the marker `[type=password]`, and workbooks carry the token
`<valid-password>`, resolved from the environment at run time.

---

<sub>Everything below this line is the upstream Playwright MCP README.</sub>
